...
class TestRoles:
    def test_total_17(self):
        assert len(build_role_list()) == 17

    def test_villager_7(self):
        assert ROLE_DEFINITIONS[RoleName.VILLAGER].count == 7

    def test_wolf_3(self):
        assert ROLE_DEFINITIONS[RoleName.WEREWOLF].count == 3

    def test_wolf_not_victim(self):
        assert not ROLE_DEFINITIONS[RoleName.WEREWOLF].can_be_first_victim

    def test_fox_not_victim(self):
        assert not ROLE_DEFINITIONS[RoleName.FOX].can_be_first_victim
...
class TestVote:
    def _state(self):
        s = GameState("t")
        for i in range(5):
            s.add_player(f"p{i}", f"P{i}", RoleName.VILLAGER)
        s.day = 2
        s.set_phase(Phase.VOTING)
        return s

    def test_majority(self):
        s = self._state()
        vm = VoteManager(s)
        for v in ["p0", "p2", "p3", "p4"]:
            vm.collect_vote(v, "p1")
    ...
    def test_tie(self):
        s = self._state()
        vm = VoteManager(s)
        vm.collect_vote("p0", "p1")
        vm.collect_vote("p1", "p0")
        vm.collect_vote("p2", "p1")
        vm.collect_vote("p3", "p0")
...
class TestVictory:
    def _s(self, alive, dead=None):
        ...
        i = 0
        for r in alive:
            s.add_player(f"p{i}", f"P{i}", r)
            i += 1
        for r in (dead or []):
            p = s.add_player(f"p{i}", f"P{i}", r)
            p.is_alive = False
            i += 1
        return s
